...
function loadData(): CivicShieldData {
  const candidatePaths = [
    path.join(process.cwd(), "civic_data (2).json"),
    JOURNAL_STORE_PATH,
    CIVIC_DATA_PATH,
    path.join(process.cwd(), "civic_data (1).json")
  ];

  let bestData: CivicShieldData | null = null;
  let bestTimestamp = 0;
    ...
    try {
      if (fs.existsSync(candidatePath)) {
        ...
        if (parsed && Array.isArray(parsed.blocks) && parsed.blocks.length > 0) {
          const timestamp = typeof parsed.lastUpdated === "number" ? parsed.lastUpdated : 1;
          if (!bestData || timestamp >= bestTimestamp) {
            bestData = parsed;
      ...
      console.warn(`Could not read candidate ${candidatePath}:`, e);
    }
  }
  ...
  // Fallback: Base initialData from source
...
// Save/Restore full campaign dataset (JSON Import or Direct Copy-Paste on Website)
app.post("/api/save-campaign-data", (req, res) => {
  try {
    let incoming = req.body?.data !== undefined ? req.body.data : req.body;
    ...
    if (!incoming || typeof incoming !== "object") {
      return res.status(400).json({ error: "Data payload must be a JSON object." });
    }

    // Merge or overwrite cleanly with validation
    campaignData = {
      ...campaignData,
      ...incoming,
      blocks: Array.isArray(incoming.blocks) && incoming.blocks.length > 0 ? incoming.blocks : campaignData.blocks,
      posts: Array.isArray(incoming.posts) ? incoming.posts : campaignData.posts,
      evidence: Array.isArray(incoming.evidence) ? incoming.evidence : campaignData.evidence,
      questions: Array.isArray(incoming.questions) ? incoming.questions : campaignData.questions,
      subscribers: Array.isArray(incoming.subscribers) ? incoming.subscribers : campaignData.subscribers,
      socialFeed: Array.isArray(incoming.socialFeed) ? incoming.socialFeed : campaignData.socialFeed,
      newsletters: Array.isArray(incoming.newsletters) ? incoming.newsletters : campaignData.newsletters,
      caseFiles: Array.isArray(incoming.caseFiles) ? incoming.caseFiles : (campaignData.caseFiles || []),
      bookReviews: Array.isArray(incoming.bookReviews) ? incoming.bookReviews : (campaignData.bookReviews || []),
      masterPasscode: incoming.masterPasscode || campaignData.masterPasscode || "lol12ymn",
      visitorStats: incoming.visitorStats ? { ...campaignData.visitorStats, ...incoming.visitorStats } : campaignData.visitorStats,
      lastUpdated: Date.now()
    };

    saveData(campaignData);
    console.log("✓ Live campaignData updated via direct JSON import/save!");
    res.json({
      success: true,
      message: "Campaign data successfully updated and saved across all storage targets!",
      data: campaignData
    });
  } catch (err: any) {
    console.error("Error in save-campaign-data:", err);
    res.status(500).json({ error: err.message });
  }
});

// List all available backup JSON files in the workspace
app.get("/api/backup-files", (req, res) => {
  try {
    const rootFiles = fs.readdirSync(process.cwd());
    const jsonBackups = rootFiles.filter(f => f.startsWith("civic_data") && f.endsWith(".json"));
    
    // Also check if data/journal_store.json exists
    const filesInfo = jsonBackups.map(filename => {
      ...
      try {
        ...
        casesCount = parsed.caseFiles?.length || 0;
        reviewsCount = parsed.bookReviews?.length || 0;
        subscribersCount = parsed.subscribers?.length || 0;
        summary = `${blocksCount} sections, ${casesCount} cases, ${reviewsCount} reviews, ${subscribersCount} subscribers`;
      } catch {
        summary = "File unparsed";
      }
      return {
        filename,
...
// Read the raw content of a specific backup file so user can load it into the copy-paste box
app.get("/api/backup-file-content", (req, res) => {
  try {
    const filename = req.query.filename as string;
    if (!filename || !filename.startsWith("civic_data") || !filename.endsWith(".json") || filename.includes("/") || filename.includes("..")) {
...
// 1-Click Restore from an existing backup file
app.post("/api/restore-backup-file", (req, res) => {
  try {
    const { filename } = req.body;
    if (!filename || !filename.startsWith("civic_data") || !filename.endsWith(".json") || filename.includes("/") || filename.includes("..")) {
      return res.status(400).json({ error: "Invalid backup filename parameter" });
    }
    const fullPath = path.join(process.cwd(), filename);
    if (!fs.existsSync(fullPath)) {
    ...
    saveData(campaignData);
    console.log(`✓ Restored dataset directly from ${filename}!`);
    res.json({ success: true, message: `Successfully applied and updated website from ${filename}!`, data: campaignData });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
});
...
app.post("/api/update-passcode", (req, res) => {
  try {
    ...
    campaignData.masterPasscode = passcode.trim();
    saveData(campaignData);
    res.json({ success: true, message: "Master passcode updated successfully!", passcode: campaignData.masterPasscode });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
});
...
Core Guidelines:
1. Answer ANY Question: Thoroughly address the user's specific prompt or situation directly and dynamically. If they ask a conversational, general, hypothetical, or follow-up question (e.g., "why do I need to know them", "how do I do this", "is it legal", "what is civic shield", "draft a notice for me"), answer that exact question directly with nuanced, high-value explanations.
2. Structured Markdown:
   - Direct clear assessment & answer
   - Relevant statutory provisions, Constitutional Articles (e.g., Art 14, 19(1)(a), 20(3), 21, 22, 32, 226), or Landmark Supreme Court Precedents (e.g., D.K. Basu, Lalita Kumari, Puttaswamy, Arnesh Kumar) when applicable
   - Step-by-step actionable advice for the citizen
   - Assertive, polite sample dialogue or scripts if relevant
3. Tone: Calm, fearless, objective, empowering, and articulate.`;

  // Tier 1: Dynamic Gemini API Integration with Multi-Model Fallback and Retry
  const geminiAi = getGeminiClient();
  if (geminiAi) {
    // Build sanitized multi-turn contents array ensuring strictly alternating user / model roles
    const formattedContents: any[] = [];
    
    if (Array.isArray(history) && history.length > 0) {
      for (const item of history.slice(-10)) {
        if (!item || !item.content || typeof item.content !== "string" || !item.content.trim()) continue;
        const normalizedRole: "user" | "model" = item.role === "model" || item.role === "assistant" ? "model" : "user";
        
        // Merge consecutive identical roles to prevent Gemini 400 Bad Request error
        if (formattedContents.length > 0 && formattedContents[formattedContents.length - 1].role === normalizedRole) {
    ...
    if (formattedContents.length > 0 && formattedContents[formattedContents.length - 1].role === "user") {
      const lastText = formattedContents[formattedContents.length - 1].parts[0].text;
      if (!lastText.includes(text.trim())) {
        formattedContents[formattedContents.length - 1].parts[0].text += `\n\nCitizen Query: ${text.trim()}`;
      }
    } else {
      formattedContents.push({
        role: "user",
    ...
    for (const modelConfig of directModels) {
  ...
  return {
    answer: `### Civic Shield Advisory: "${text.trim()}"\n\n` +
      `1. **Statutory Due Process:** Every official action or directive must have backing in enacted law or procedural guidelines. You have the right to ask: *"Officer/Official, could you please cite the specific statutory section or rule under which this is requested?"*\n` +
      `2. **Right to Record & Observe:** Citizens may peacefully record interactions in public spaces without interfering with official duties (Article 19(1)(a)).\n` +
      `3. **Protection of Privacy & Self-Incrimination:** You cannot be compelled to unlock devices or provide self-incriminating statements without a lawful warrant or magistrate order (Articles 20(3) & 21).\n` +
      `4. **Remedies & Redressal:** If your rights are compromised, remedies include filing a First Information Report (FIR), approaching the Magistrate under Section 156(3) CrPC, or initiating a Writ Petition under Article 226/32.`,
  ...
  try {
    ...
    if (aiResult && aiResult.answer) {
      ...
      if (aiResult.sources && aiResult.sources.length > 0) {
        newQuestion.sources = aiResult.sources;
      }
    } else {
